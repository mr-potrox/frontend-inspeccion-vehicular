...
export default function ProgressStepper({ steps, active }: { steps: string[]; active: number }) {
  return (
        ...
        {steps.map((s, i) => {
          ...
          return (
            <div key={s} className="flex-1">
              ...
              {i < steps.length - 1 && <motion.div initial={{ width: 0 }} animate={{ width: '100%' }} className={`h-1 mt-2 rounded ${i < active ? 'bg-green-300' : 'bg-gray-200'}`} />}
            </div>
          )
        })}
      </div>
